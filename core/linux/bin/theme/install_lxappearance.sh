...
Terminal=false"
    ...
    if [[ *"${DESKTOP_PATH}"* == *"home"* ]]; then
        # ~/.local/share/applications/com.github.maoschanz.lxappearance.desktop
        su - ${CUR_USER} -c "echo \"${DESKTOP_CMD}\" > ${DESKTOP_PATH}";
    else
        # /usr/share/applications/com.github.maoschanz.lxappearance.desktop
        echo "${DESKTOP_CMD}" > ${DESKTOP_PATH};
    fi
}


function install_lxappearance_for_nix()
{
    ...
    # 3) install_lxappearance --------------------------------------------------
    # https://search.nixos.org/packages
    # nix-env -iA nixpkgs.lxappearance
    # nix profile add nixpkgs#lxappearance
    ...
    # 7) etc -------------------------------------------------------------------
    # ~/.nix-profile/share/lxappearance
    # --------------------------------------------------------------------------
}
# ==============================================================================
...
if [[ "${BASH_SOURCE[0]}" == "${0}" ]]; then
    ...
    if [[ *"${CUR_VER}"* == *"archlinux"* ]]; then
        ...
        # ----------------------------------------------------------------------
        # distrobox를 사용한다.
        # echo "lxappearance is not supported for RHEL"

        install_lxappearance_for_nix "single";
        # ----------------------------------------------------------------------
