...
[[ -n "${_INSTALL_UBUBOX_FUNCS_LOADED:-}" ]] && return 0
_INSTALL_UBUBOX_FUNCS_LOADED=1
...
function get_ctr_args()
{
    ...
    # --------------------------------------------------------------------------
    # distrobox create --name "ububox-main" --image "docker.io/library/ubuntu:latest"
    local ctr_args=""
...
function get_pre_init_hooks()
{
    ...
    # development tools
    pre_init_hooks+=" && \
        sudo apt install -y --no-reinstall build-essential"

    pre_init_hooks+=" && \
        sudo apt install -y --no-reinstall autoconf automake libtool pkg-config"
    # --------------------------------------------------------------------------
...
function install_chromium()
{
    ...
    # --------------------------------------------------------------------------
    # installation
    distrobox enter "${ctr_name}" -- sudo apt install -y --no-reinstall chromium-browser
