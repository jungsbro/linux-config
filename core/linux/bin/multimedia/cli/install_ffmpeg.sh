...
function execute_main()
{
    if [[ "${CUR_RELEASE}" == *"archlinux"* ]]; then
        ...
        # ffmpeg-devel
        local app_name="libavcodec-dev"; apt-cache show "${app_name}" &>/dev/null && apt install -y --no-reinstall "${app_name}" || true
        local app_name="libavformat-dev"; apt-cache show "${app_name}" &>/dev/null && apt install -y --no-reinstall "${app_name}" || true
        # ----------------------------------------------------------------------
