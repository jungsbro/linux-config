...
Terminal=false"
    ...
    if [[ *"${DESKTOP_PATH}"* == *"home"* ]]; then
        # ~/.local/share/applications/com.github.maoschanz.i3blocks.desktop
        su - ${CUR_USER} -c "echo \"${DESKTOP_CMD}\" > ${DESKTOP_PATH}";
    else
        # /usr/share/applications/com.github.maoschanz.i3blocks.desktop
        echo "${DESKTOP_CMD}" > ${DESKTOP_PATH};
    fi
}


function install_i3blocks_for_nix()
{
    ...
    # nix-env -iA nixpkgs.i3blocks
    # nix profile add nixpkgs#i3blocks
...
if [[ "${BASH_SOURCE[0]}" == "${0}" ]]; then
    ...
    if [[ *"${CUR_VER}"* == *"archlinux"* ]]; then
        ...
        # ----------------------------------------------------------------------
        # distrobox를 사용한다.
        # echo "i3blocks is not supported for RHEL"

        install_i3blocks_for_nix "single";
        # ----------------------------------------------------------------------
    fi
